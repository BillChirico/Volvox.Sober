...
export const getTopCompatibilityFactors = (
  ...
  const factorArray = (Object.keys(factors) as Array<keyof CompatibilityFactors>).map(key => ({
    factor: key,
    score: factors[key],
 ...
 */
export const calculateProgramCompatibility = (program1: string, program2: string): number => {
  // Exact match
