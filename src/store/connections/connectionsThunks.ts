...
export const fetchActiveConnections = createAsyncThunk(
  ...
  async (userId: string, { dispatch, rejectWithValue }) => {
    try {
      ...
      const message =
        error instanceof Error ? error.message : 'Failed to fetch active connections'
...
export const fetchEndedConnections = createAsyncThunk(
  ...
  async (userId: string, { dispatch, rejectWithValue }) => {
    try {
      ...
      const message =
        error instanceof Error ? error.message : 'Failed to fetch ended connections'
      dispatch(setError(message))
      return rejectWithValue(message)
    } finally {
      dispatch(setLoading(false))
...
export const refreshAllConnections = createAsyncThunk(
  ...
  async (userId: string, { dispatch, rejectWithValue }) => {
    try {
      ...
      const message =
        error instanceof Error ? error.message : 'Failed to refresh connections'
      dispatch(setError(message))
      return rejectWithValue(message)
    } finally {
...
export const updateUnreadCount = createAsyncThunk(
  ...
  async (
    { connectionId, count }: { connectionId: string; count: number },
    { dispatch }
  ) => {
