...
class MatchingService {
   ...
   */
  calculateCompatibilityScore(userCriteria: MatchCriteria, candidate: Profile): MatchScore {
    const factors: CompatibilityFactors = {
      ...
      locationProximity: this.calculateLocationProximity(userCriteria.location, {
        city: candidate.city,
      ...
      experienceLevel: this.calculateExperienceLevel(candidate.sobriety_start_date),
    };
  ...
  private calculateRecoveryProgramMatch(userProgram: string, candidateProgram: string): number {
  ...
  private calculateAvailabilityOverlap(
    ...
    if (userAvailability.includes('anytime') || candidateAvailability.includes('anytime')) {
      return 100;
    }

    const overlap = userAvailability.filter(slot => candidateAvailability.includes(slot));
