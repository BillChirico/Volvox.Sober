...
const matchesSlice = createSlice({
  ...
  reducers: {
    ...
    removeSuggestedMatch: (state, action: PayloadAction<string>) => {
      state.suggestedMatches = state.suggestedMatches.filter(match => match.id !== action.payload);
    },
    moveToRequested: (state, action: PayloadAction<string>) => {
      const match = state.suggestedMatches.find(m => m.id === action.payload);
      if (match) {
        state.suggestedMatches = state.suggestedMatches.filter(m => m.id !== action.payload);
