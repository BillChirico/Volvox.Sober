...
class MessageServiceV2 {
  ...
  async getConversation(
    ...
    try {
      ...
      // Get messages
      const { data: messages, error: msgError } = await this.getMessages(
        connectionId
      )
  ...
  async markAsDelivered(
    data: MarkMessagesDeliveredData
  ): Promise<{ error: Error | null }> {
    try {
      const updateData: TablesUpdate<'messages'> = {
        status: 'delivered',
      ...
      .on(
        ...
        payload => {
          ...
          onStatusUpdate({
            ...
            timestamp:
              message.read_at || message.delivered_at || message.created_at,
          })
        }
      )
      .subscribe()
