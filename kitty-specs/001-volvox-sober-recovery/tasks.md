...
## Work Package WP03: User Onboarding & Profile Management (Priority: P1) 🔧 Foundation
...
## Work Package WP04: Matching Algorithm & Match Display (Priority: P1) 🎯 MVP Core
...
- Sobriety alignment: sponsors should have ≥ 2x sponsee's sobriety time for mentorship credibility
...
## Work Package WP05: Connection Requests & Management (Priority: P1) 🎯 MVP Core
...
- Connection request workflow: Sponsee sends → Sponsor reviews → Accept creates active connection OR Decline with optional reason
...
- Connection status: pending → active (on accept) OR pending → declined (on decline)
...
## Work Package WP09: Push Notifications & Real-Time Updates (Priority: P2) 🔔 Cross-Cutting
...
- [ ] T134 Implement notification triggers for connection requests (INSERT on connection_requests → send to sponsor)
- [ ] T135 [P] Implement notification triggers for request accepted/declined (UPDATE on connection_requests → send to sponsee)
- [ ] T136 [P] Implement notification triggers for new messages (INSERT on messages → send to recipient)
- [ ] T137 [P] Implement notification triggers for milestone celebrations (milestone job → send to user and connected sponsors/sponsees)
- [ ] T138 [P] Implement notification triggers for relapse alerts (INSERT on relapses → send to connected sponsors)
- [ ] T139 [P] Implement notification triggers for check-in reminders (check-in job → send to sponsee)
...
```
WP01 (Setup)
  → WP02 (Database & Auth Foundation)
    → WP03 (User Onboarding & Profiles)
      → WP04 (Matching Algorithm) [P1 MVP]
      → WP05 (Connection Requests) [P1 MVP]
        → WP06 (Sobriety Tracking) [P2]
        → WP07 (12-Step Worksheets) [P2]
        → WP08 (Messaging & Check-Ins) [P2]
        → WP09 (Notifications & Realtime) [P2 Cross-Cutting]
      → WP10 (Dark Mode & Accessibility) [P3]
```
