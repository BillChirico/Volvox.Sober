...
import type { CheckIn, CheckInResponse } from '../../../src/types'
...
export const CheckInResponseScreen: React.FC<CheckInResponseScreenProps> = ({
  ...
  const isFormValid = (): boolean => {
    ...
    const unansweredQuestions = checkIn.questions.filter(
      (q: string) => !answers[q] || answers[q].trim().length === 0
    )
  ...
  const handleSubmit = async () => {
    ...
    Alert.alert(
      'Submit Check-In',
      'Are you ready to submit your check-in response?',
      [
      ...
      <ScrollView style={styles.scrollContainer} contentContainerStyle={styles.content}>
        ...
        <View style={styles.progressContainer}>
          ...
          <Text style={styles.progressText}>
            {Object.values(answers).filter(a => a.trim().length > 0).length} of {checkIn.questions.length} answered
          </Text>
        </View>
              ...
              <View key={response.id} style={styles.historyItem}>
                <View style={styles.historyItemHeader}>
                  <Text style={styles.historyItemDate}>
                    {formatDate(response.created_at || response.scheduled_for)}
                  </Text>
                  <View style={[
                    styles.historyItemStatus,
                    response.response_status === 'completed' ? styles.historyItemStatusCompleted : styles.historyItemStatusMissed
                  ]}>
                    <Text style={styles.historyItemStatusText}>
                      {response.response_status === 'completed' ? '✓ Completed' : '✗ Missed'}
                    </Text>
                  </View>
                </View>
              </View>
