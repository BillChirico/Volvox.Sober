...
test.describe('Matching Flow', () => {
  ...
  test('T143.2: Filter matches by recovery program', async ({ page }) => {
    ...
    for (let i = 0; i < count; i++) {
      const programText = await matchCards.nth(i).locator('[data-testid="match-program"]').textContent()
  ...
  test('T143.3: Filter matches by availability', async ({ page }) => {
    ...
    if (count > 0) {
      for (let i = 0; i < Math.min(count, 3); i++) {
        const availText = await matchCards.nth(i).locator('[data-testid="match-availability"]').textContent()
  ...
  test('T143.5: Send connection request from match profile', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="send-request-button"]')).toContainText(
      'Request Sent'
    )
  ...
  test('T143.9: Match compatibility score calculation', async ({ page }) => {
    ...
    const overallScore = await page.locator('[data-testid="overall-compatibility-score"]').textContent()
