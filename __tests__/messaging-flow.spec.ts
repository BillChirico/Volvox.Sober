...
test.describe('Messaging Flow', () => {
  ...
  test('T115.1: Send message to active connection', async ({ page }) => {
    ...
    ).toContainText('sent', { timeout: 5000 })

    // Verify conversation preview updated with last message
    await page.click('[data-testid="back-button"]')
    await expect(
      page.locator('[data-testid="conversation-item-0"]')
    ).toContainText(messageText.substring(0, 50))
  })
  ...
  test('T115.2: Receive message via Realtime subscription', async ({ page, context }) => {
    ...
    await expect(
      sponseePage.locator('[data-testid="message-bubble"]').last()
    ).toContainText(messageText, { timeout: 1000 })
    ...
    await expect(
      sponseePage.locator('[data-testid="tab-messages-badge"]')
    ).toBeVisible()
  ...
  test('T115.5: Read receipt tracking', async ({ page, context }) => {
    ...
    await expect(
      sponsorPage.locator('[data-testid="message-status"]').last()
    ).toContainText('sent', { timeout: 5000 })
    ...
    await expect(
      sponsorPage.locator('[data-testid="message-status"]').last()
    ).toContainText('read', { timeout: 2000 })
  ...
  test('T115.6: Message pagination (load more)', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="loading-more-indicator"]')
    ).toBeVisible()
  ...
  test('T115.7: Message validation (max length)', async ({ page }) => {
    ...
    ).toContainText('Message cannot exceed 5000 characters')
