...
async function verifyColorContrast(
  ...
  expect(contrast, `${elementName} contrast ratio should be at least ${minContrast}:1 (got ${contrast.toFixed(2)}:1)`).toBeGreaterThanOrEqual(minContrast)
}
...
test.describe('T136: Touch Target Size Verification', () => {
  ...
  test('T136.1: Login screen touch targets', async ({ page }) => {
    ...
    await verifyTouchTargetSize(page, '[data-testid="forgot-password-link"]', 'Forgot password link')
  })
  ...
  test('T136.3: Match card action buttons', async ({ page }) => {
    ...
    if (await viewProfileButton.count() > 0) {
      await verifyTouchTargetSize(page, '[data-testid="view-profile-button"]', 'View profile button')
    }
  })
  ...
  test('T136.5: Message send button', async ({ page }) => {
    ...
    if (await sendButton.count() > 0) {
      await verifyTouchTargetSize(page, '[data-testid="send-message-button"]', 'Send message button')
    }
  })
...
test.describe('T137: Color Contrast Verification (WCAG 2.1 AA)', () => {
  ...
  test('T137.4: Error message contrast', async ({ page }) => {
    ...
    if (await errorMessage.count() > 0) {
      await verifyColorContrast(page, '[data-testid="error-message"], [role="alert"]', 'Error message')
    }
  })
  ...
  test('T137.5: Secondary text contrast', async ({ page }) => {
    ...
    const helperTexts = page.locator('[data-testid*="helper"], [data-testid*="subtitle"], [data-testid*="caption"]')
  ...
  test('T137.6: Placeholder text contrast', async ({ page }) => {
    ...
    const contrast = calculateContrastRatio(placeholderColor.color, placeholderColor.backgroundColor)
