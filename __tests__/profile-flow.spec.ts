...
test.describe('Profile Management Flow', () => {
  ...
  test('T125.2: Edit profile information', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="profile-location"]')).toContainText('San Francisco, CA')
  })
  ...
  test('T125.3: Change role from sponsor to sponsee', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="profile-support-needs"]')).toContainText('Accountability')
